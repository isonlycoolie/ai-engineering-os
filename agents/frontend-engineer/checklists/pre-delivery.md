...
- [ ] Feature code scoped to `features/<feature>/` - no cross-feature deep imports
...
- [ ] Server data fetched via TanStack Query only - no `useEffect` fetching
- [ ] Server state not stored in Zustand
...
- [ ] TailwindCSS for styling - no inline styles except dynamic values
...
- [ ] No secrets or inappropriate values in `NEXT_PUBLIC_` env vars
